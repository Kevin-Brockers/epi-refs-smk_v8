...
def compute_log2fc_data(processing_dict='', 
    drop_na=drop_na,
    out_path=''
    ):
    _concat_df_cpm = []
    ...
    for key, path in processing_dict.items():
        ...
        df = load_bedgraphs(path=path,
                            col_label=key)
        _concat_df_cpm.append(df)

    logging.info('Concat data')
    _concat_df_cpm = pd.concat(_concat_df_cpm, 
        axis=1)
...
compute_log2fc_data(processing_dict=processing_dict,
