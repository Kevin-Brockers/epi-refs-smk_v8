...
- [x] Implement peak calling
  - [x] Peak calling uses macs3 and currently only works when config['AGGREGATE_REPLICATES] == 'sum'
  - [x] Also currently only works for "single end" reads, independent whether SE sequenced or PE sequenced and only one
